...
final class LongPollingActivateJobsPhysicalTenantTest {
  ...
  void shouldNotWakeRequestOfAnotherPhysicalTenant() {
    ...
    submitPendingRequest("tenant-a", tenantACompletions);
    submitPendingRequest("tenant-b", tenantBCompletions);

    // when — a job becomes available and only tenant-a's topic is notified
    activateJobsStub.addAvailableJobs(TYPE, 1);
    brokerClient.notifyJobsAvailable("tenant-a-jobsAvailable", TYPE);
  ...
  void shouldSubscribeOncePerPhysicalTenantAcrossMultipleRequests() {
    ...
    submitPendingRequest("tenant-a", new AtomicInteger());
    submitPendingRequest("tenant-a", new AtomicInteger());
    ...
    assertThat(brokerClient.getJobAvailableSubscriptionTopics())
        .filteredOn("tenant-a-jobsAvailable"::equals)
        .hasSize(1);
  }
