...
final class OldGatewayNewBrokerJobStreamingTest {
  ...
  @BeforeEach
  void assumeUpgradePathIsSupported() {
    final var previous = VersionUtil.getPreviousSemanticVersion().orElseThrow();
    final var current = VersionUtil.getSemanticVersion().orElseThrow();
    Assumptions.assumeTrue(
        VersionCompatibilityMatrix.isCompatible(previous, current),
        () ->
            "skipping: %s -> %s is a known-incompatible rolling upgrade path"
